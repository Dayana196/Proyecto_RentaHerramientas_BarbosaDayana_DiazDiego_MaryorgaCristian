document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderCards(lista) {
        grid.innerHTML = "";
        lista.forEach(item => {
            const esDisponible = item.estado === "DISPONIBLE";
            const btnState = esDisponible ? "" : "disabled";
                        ...
                        <div class="product-overlay">
                            <div class="overlay-details">
                                ...
                                <div class="button-group">
                                    <button class="add-to-cart" ${btnState}><i class="fa-solid fa-bolt"></i> ALQUILAR</button>
                                    <button class="btn-reserve" ${btnState}><i class="fa-solid fa-calendar-plus"></i> RESERVAR</button>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    grid.addEventListener('click', (e) => {
        const btn = e.target.closest('.add-to-cart') || e.target.closest('.btn-reserve');
        ...
        if (btn && !btn.hasAttribute('disabled')) {
            ...
            document.getElementById('modal-title').innerText = btn.classList.contains('btn-reserve') ? "RESERVAR EQUIPO" : "ALQUILAR EQUIPO";
            document.getElementById('display-item-name').innerText = nombre;
            document.getElementById('display-item-price').innerText = precioNumerico.toFixed(2);
            document.getElementById('display-deposit').innerText = (precioNumerico / 2).toFixed(2);

            modal.style.display = "block";
        } else if (btn && btn.hasAttribute('disabled')) {
            console.log("Este equipo no está disponible actualmente.");
        }
    });
    ...
    rentalForm.onsubmit = (e) => {
        ...
        const nuevoAlquiler = {
            equipo: document.getElementById('display-item-name').innerText,
            cliente: document.getElementById('client-name').value,
            email: document.getElementById('client-email').value,
            metodo: document.getElementById('payment-method').value,
            abono: document.getElementById('display-deposit').innerText,
            momento: document.getElementById('payment-time').value
        };

        let baseDeDatos = JSON.parse(localStorage.getItem('alquileresToolsX')) || [];
        baseDeDatos.unshift(nuevoAlquiler);
        localStorage.setItem('alquileresToolsX', JSON.stringify(baseDeDatos));

        alert("¡Registro exitoso! Ya puedes revisar " + nuevoAlquiler.equipo + " en tu sección de Alquilados.");
    ...
    filterBtns.forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            renderCards(cat === "TODAS" ? inventario : inventario.filter(i => i.categoria === cat));
        });
    });
    ...
    statusBtns.forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            filterBtns.forEach(b => b.classList.remove('active'));
            renderCards(inventario.filter(i => i.estado === estado));
        });
    });

    renderCards(inventario);
});